...
- `covar1 + covar2`: fit the intercept, the parameter of `covar1` and
  the parameter of `covar2`.
- `covar1:covar2`: fit the intercept and the interaction between
  `covar1` and `covar2`.
- `covar1 * covar2`: equivalent to `covar1 + covar2 + covar1:covar2`
  (fit the intercept, the parameter of `covar1`, the parameter of
  `covar2` and the interaction between `covar1` and `covar2`).

This is based the reference R implementation of formulas. [Learn
more](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/formula.html).
...
---
mortality:
  expt_death: geno + type + geno:type
  survival_param2: 1
