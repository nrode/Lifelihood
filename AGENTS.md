# Instruction for LLMs when working on lifelihood
...
- Github issues live here: <https://github.com/nrode/Lifelihood/issues>
...
- Use Air (`air format . --check`) and Jarl (`jarl check .`) to check
  for code quality
- Always use devtools and never pkgdown or testthat
- Never manually updates \*.rd files, always use
  [`devtools::document()`](https://devtools.r-lib.org/reference/document.html)
- Use
  [`devtools::load_all()`](https://devtools.r-lib.org/reference/load_all.html)
  for loading latest version
- Use
  [`devtools::test()`](https://devtools.r-lib.org/reference/test.html)
  for testing
- Since testing suite takes a lot of time, only test relevant files
  unless it’s necessary
- Do not make changes other than the one asked. If you find issues in
  code, mention them as the end of your task and summarize what is wrong
  and could we possibly fix it.
...
\
`df`` ``<-`` ``fakesample`\
[`head`](https://rdrr.io/r/utils/head.html)`(``df``)`\
`df``$``type`` ``<-`` `[`as.factor`](https://rdrr.io/r/base/factor.html)`(``df``$``type``)`\
`df``$``geno`` ``<-`` `[`as.factor`](https://rdrr.io/r/base/factor.html)`(``df``$``geno``)`\
\
