...
const
  ...
  unsurtwopi = 0.15915494309189535;     {1/(2Pi))}
...
function Expo(X: double): double; overload;
begin
  MathError := FN_OK;
  if X < MINLOG then
...
begin
  ...
  begin
    ...
    begin
      ...
      WAPR := RETA / (1.0 + RETA / (3.0 + (LC.S21 * AN2 + LC.S22) *
        RETA / (LC.S23 * (AN2 + RETA)))) - 1.0;
    end
    else
    begin
      ZL := Ln(XX);
      WAPR := Ln(XX / Ln(Power(XX / ZL, Exp(-1.124491989777808 /
  ...
  begin
    ...
    begin
      ...
      TS := Sqrt(T);
      WAPR := ZL - (2.0 * TS) /
        (SQRT2 + (LC.C13 - T / (2.7E2 + TS * 127.0471381349219)) * TS);
    end
    else
    begin
      ZL := Ln(-XX);
