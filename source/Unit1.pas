...
procedure RaiseReadContext(const SourcePath, Section: string; LineNo: integer;
  const RawLine: string; E: Exception);
var
  Message: string;
begin
  Message := 'while reading';
  if Section <> '' then Message := Message + ' ' + Section;
  if SourcePath <> '' then Message := Message + LineEnding + '  file: ' + SourcePath;
  if LineNo > 0 then Message := Message + LineEnding + '  line: ' + IntToStr(LineNo);
  Message := Message + LineEnding + '  cause: ' + E.ClassName + ': ' + E.Message;
  if RawLine <> '' then Message := Message + LineEnding + '  last_line: ' + RawLine;
...
begin
  ...
  try
    reset(f1);
    section := 'data struct header';
    readln(f1);    //line 'data struct'
    lineNo := lineNo + 1;
    lal := TStringList.Create;
    lal.Delimiter := ' ';
    section := 'matclutch declaration';
    ...
    begin
      section := 'model parameter row ' + IntToStr(j + 1);
      readln(f1, check);
      lineNo := lineNo + 1;
      lal.DelimitedText := check;
    ...
    begin
      ...
      begin
        ...
        begin
          if lal[j] = 'sex' then
          begin
            section := 'sex event on life-history data row';
            numevent := numevent + 1;
            setlength(
          ...
          if lal[j] = 'mat' then
          begin
            section := 'maturity event on life-history data row';
            numevent := numevent + 1;
            setlength(
              gi[i].LH[numhv - 1].events, numevent);
            gi[
          ...
          if lal[j] = 'pon' then
          begin
            section := 'clutch event on life-history data row';
            numevent := numevent + 1;
            setlength(
              gi[i].LH[numhv - 1].events, numevent);
            gi[
          ...
          if lal[j] = 'mor' then
          begin
            section := 'mortality event on life-history data row';
            numevent := numevent + 1;
            gi[
    ...
    section := 'generating non-clutch events';
    Gnop;

    // Gerer les juveniles non sexés
    section := 'handling unsexed juveniles';
    ...
    section := 'building event calendars';
    for i := 0 to nb_group - 1 do for j := 0 to group[i].nb_ind - 1 do
    ...
    on E: Exception do
      RaiseReadContext(nomf1, section, lineNo, check, E);
  end;
end;
...
begin
  lineNo := 0;
  section := 'opening parameter bounds file';
  check := '';
  try
    ...
    for i := 0 to nbparposs - 1 do
      ...
      begin
        section := 'parameter bounds row ' + IntToStr(i + 1);
        readln(fc, check);
        lineNo := lineNo + 1;
        lal.DelimitedText := check;
        Name := lal[0];
    ...
    on E: Exception do
      RaiseReadContext(nomfc, section, lineNo, check, E);
  end;
end;
...
begin
  lineNo := 0;
  section := 'continuous covariate values';
  check := '';
  try
    if flag then
    begin
      ...
      for i := 1 to nbcov do
      begin
        section := 'continuous covariate row ' + IntToStr(i);
    ...
    on E: Exception do
      RaiseReadContext(path_continuous_var, section, lineNo, check, E);
  end;
