...
type
  var_info_type = record
    ...
    //echantilloner distrib du param pour estimer supp lim
    Name: string;
    typ, loc: integer;
  end;
  ...
  groupe_info = record
    ...
    use, nb_ind: integer;
    //name: string;
    matsanspon, mat, mort, ponte: survfunctype;
    //quand matclutch = true 'mat' refere au temps de mat + 1ere ponet, ce n'est pas a proprement parlé 'mat', d'ou cette survfunctype matsanspon pour le calcul propre de la fitness ds ce cas
...
var
  ...
  H: mat;
  intervalp1p2: double;
  //intervale de temps entre 1ere et 2eme ponte pour tous les individus, utile quand matclutch=true pour avoir une idee moyenne de l'intervale entre maturite reelle et premiere ponte
  savedseed: array[1..4] of integer;
  suaux: survfunctype;
  continuous_var_flag: boolean; //tell if there is a file with valcont values
...
{----------------------------------------------------------------------------------------------------}
function getgroup(v: vecti): integer;
  //obtiens le num du groupe a partir des valeurs des cov individuelles
begin
  case nbcov of
    0: getgroup := 0;
...
{--------------------------------version originale--------------------------------------------------------------------}
function getcov(group, covnum: integer): integer;
  // renvoie la valeur du facteur correpondant a la covariable covnum pour le groupe numero 'group'
var
  a, help: integer;
begin
  if covnum < (nbcov + 1) then help := covnum
  else
    help := covnum - nbcov;    //si cov continue, donne le bon numero
  case nbcov of
    0: getcov := 0;
    1: getcov := group;
    2: case help of
...
begin
  if x > 200 then link := pt.maxbound - minus
  else
  if x < -200 then link := pt.minbound + minus
  else
...
begin
  if val <= pt.minbound then
  begin
    //messageform.Memo2.Lines[6]:=floattostr(val);
    delink := -200;
  end
  else if val >= pt.maxbound then
  begin
...
begin
  test := 1;
  for i := 1 to hv.nb_event - 1 do
    if (hv.events[i].t1 < hv.events[i - 1].t1) and (hv.events[i].t2 < tinf) and
      (hv.events[i - 1].t2 < tinf) then test := test * 0;
  if test = 0 then check := False
  else
    check := True;
end;
...
begin
  ...
  begin
    ...
    for i := 0 to nb_event - 1 do if (events[0].t2 = tinf) or (events[i].t2 = tinf) then
        interv[i] := 1
      else
        interv[i] := round((events[i].t2 - events[i].t1) / sub_interval);
    {les hv ou mat est censurée: pas de decoupage (ni mat ni mort) et les morts censuree non decoupées aussi }
    setlength(diviseur, nb_event);
    diviseur[nb_event - 1] := 1;
    for i := 0 to nb_event - 2 do diviseur[nb_event - 2 - i] :=
        diviseur[nb_event - 1 - i] * interv[nb_event - 1 - i];
    ...
    begin
      ...
      begin
        hvvirt.events[j].t1 :=
          events[j].t1 + pos[j] * ((events[j].t2 - events[j].t1) / interv[j]);
        hvvirt.events[j].t2 :=
          events[j].t1 + (pos[j] + 1) * ((events[j].t2 - events[j].t1) / interv[j]);
        hvvirt.events[j].Name := events[j].Name;
        if events[j].Name = 'pon' then
          hvvirt.events[j].tp := events[j].tp;
      end;
      if check(hvvirt) then
      begin
...
var
  i, j, k, numevent: integer;
begin
  for i := 0 to nb_group - 1 do
    for j := 0 to group[i].nb_ind - 1 do
      for k := 0 to group[i].gi[j].nb_hv - 1 do
        if (group[i].gi[j].lh[k].events[1].t2 < tinf) and
          (group[i].gi[j].lh[k].events[0].tp = 0) then
        begin              //c'est une femelle qui a maturé
          numevent :=
            group[i].gi[j].lh[k].nb_event + 1;
          group[i].gi[j].lh[k].nb_event :=
            numevent;
          setlength(
            group[i].gi[j].lh[k].events, numevent);
          {on ajoute evenement non ponte si il y a eu maturite}
          group[i].gi[j].lh[
            k].events[numevent - 1].Name := 'nop';
          group[i].gi[j].lh[
            k].events[numevent - 1].t1 := group[i].gi[j].lh[k].events[numevent - 2].t1;
          {    nop.t1 = mort.t1  }
          group[i].gi[j].lh[
            k].events[numevent - 1].t2 := tinf;
          {on censure artificiellement l'evenement non ponte par definition}
          //NB: si nop.t1 est negatif (quand mort et derniere ponte ds le meme intervale) alors c'est pas grave car les fonction surv sont protegée, ca donne une proba de 1 a l'evenement nop qui reflete l'absence d'info dans ce cas (on ne sait rien avec certitude: par exemple la mort peut intervenir au tout debut de l'intervale, ce qui correspondrait a un t1 de nonponte de zero)
        end;
end;
...
var
  t: double;
begin
  if x > 3.5 then erfrapos := 1 - minus
  else
  begin
    t := 1 / (1 + p_erfra * x);
    erfrapos := 1 - (a1_erfra * t + a2_erfra * t * t + a3_erfra * t * t * t) *
      Exp(-(x * x));
  end;
end;
...
begin
  if x > 0 then erfra := erfrapos(x)
  else
    erfra := -erfrapos(-x);
end;

{ ************************************************************************ }
function Incomplete_gamma_mathematica(a, b: double): double;
  //Verifié c'est bien Gamma[a,b] mathematica
begin
  Incomplete_gamma_mathematica := JGamma(a, b);
end;

{ ************************************************************************ }
function gamma_regularized_mathematica(a, b: double): double;
  //Verifié c'est bien GammaRegularized[a,b] mathematica
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if Name = 'exp' then
        //ici hazard constant = 1/vp0  ; esperance du tps de survie est vp0
      begin
        if vp[0] <= 0 then vp[0] := eps;
        if x <= 0 then  surv := 1
        else if (sex = 1) and (vp[2] > 0) then surv := Expo(-(x / (vp[0] * vp[2])))
        else
          surv := Expo(-(x / vp[0]));
      end;

      if Name = 'wei' then
        //hazard est weibull, d'esperance vp0 ici. En version mathematica Weibull[vp1,paramaux] et esperance de cette loi vaut paramaux Gamma[1+1/vp1]
      begin
        if vp[0] <= 0 then  vp[0] := eps;
        if vp[1] <= 0 then  vp[1] := eps;
        paramaux := vp[0] / Gamma(1 + 1 / vp[1]);
        //donc ici vp[0] est l'esperance du temps d'attente de l'evenement
        if x <= 0 then  surv := 1
        else if (sex = 1) and (vp[2] > 0) then
          surv := Expo(-(Power((x / (paramaux * vp[2])), vp[1])))
        else
          surv := Expo(-(Power((x / paramaux), vp[1])));
      end;

      if Name = 'lgn' then
      begin
        if vp[0] <= 0 then  vp[0] := eps;
        //donc ici vp[0] est aussi l'esperance du temps d'attente de l'evenement
        if vp[1] <= 0 then  vp[1] := eps;
        if x <= 0 then  surv := 1
        else
        begin
          if (sex = 1) and (vp[2] > 0) then
            paramaux := Ln(vp[0] * vp[2]) - 0.5 * vp[1] * vp[1]
          // esperance LgNormal(mu,sig) = Exp[mu+sig2/2]     //esperance x vp[2] si c'est un male
          else
            paramaux := Ln(vp[0]) - 0.5 * vp[1] * vp[1];
          // esperance LgNormal(mu,sig) = Exp[mu+sig2/2]
          surv := 0.5 * (1 + erfra((paramaux - ln(x)) / (vp[1] * racine2)));
          //surv = 1-CDF[LogNormale[mu,sig],x]  ou mu == paramaux  (on le calcule en reparametrant avec l'esperance vp0
        end;
      end;
      ...
      begin
        ...
        if x <= 0 then  surv := 1
          ...
          paramaux := (vp[0] * vp[2]) / vp[1]
        // esperance gamma(a,b) = ab          b = 1/paramaux
        else
          paramaux := vp[0] / (vp[1]);
        surv := gamma_regularized_mathematica(paramaux, x / vp[1]);
        //surv = 1-CDF[Gamma[a,b],x] == 1-GammaRegularized[a,0,x/b] ==  GammaRegularized[a,x/b]   //CHECKé PENDANT 2 JOURS!!!!
      end;
...
begin
  if su.vp[3] > 0 then
  begin
    if (x < tc) then survp := 1
    else
      survp := su.vp[3] * surv(x, su, sex);
  end
  else
    survp := surv(x, su, sex);
end;
...
{ ************************************************************************* }
function CalculRatioEspPoissonTronque(var ev: event_type; var pon: survfunctype;
  var hv: life_history): double;
var
  sum, tfrommat: double;
begin
  if ev.Name = 'pon' then  tfrommat := ev.fin - hv.events[1].fin
  else
  ...
  if sum > 20 then CalculRatioEspPoissonTronque := ratiomax
    ...
    CalculRatioEspPoissonTronque := ratiomax / (1 + Exp(xratiomax - sum));
  //ajusté pour valoir 1 quand sum=0: donc pas d'effet si vp8,vp9,vp10 sont nuls  //attention esp d'une loi de poisson tronquée ne peut pas etre <1     d'ou la condition sur espPoissontronque*ratio dans la procedure probevent
end;

{ ************************************************************************* }
function Survtotpon(tfromlastpon, tfrommat: double; var su: survfunctype;
...
begin
  ...
  if (su.vp[6] <> 0) or (su.vp[7] <> 0) then
  begin
    suaux.Name := su.Name;
    suaux.vp[1] := su.vp[1];
    ...
    suaux.vp[0] := link(vp0 + tfrommat * (vp6 + tfrommat * vp7), LL_D.paramdescript[8]);
    //change la moyenne de la fonction su localement en y mettant un effet du tfrommat
    ...
    Survtotpon := surv(tfromlastpon, su, sex);

end;

{ ************************************************************************* }
function Sda(t, d, da, dn: double; var lh: life_history; sex: integer): double;
  //fonction qui corrige S(t) en ajoutant le hazard cumulé du fait d'un trade off survie repro : la bonne fonction de survie devient le produit S(t) Sda(t,d,lh). Mais ici le l'increment de hazard a chaque ponte (d+dn*taille ponte) est amorti exponentiellement au taux da.
var
  i, aux: integer;
  sumhaz, ttodeath: double;
begin
  if matclutch = 1 then aux := 0
  else
    aux := 1;
  if lh.nbponte = 0 then Sda :=
      1  //pas de ponte     // les males n'ont pas de ponte de toutes les facons
  else
  begin
    sumhaz := 0;
    for i := 1 to lh.nbponte do
    begin
      ttodeath := (t - lh.events[i + aux].fin);
      //aux = 1 :commence a la premiere  ponte dont l'indice est event[2]    //aux = 0 : commence a la maturite qui est une ponte et dont l'indice est event[1]
      if ttodeath < 0 then ttodeath := 0;
      //WARNING dire qu'il y a un pb ds les données    //mort est le dernier evenement comme ca devrait, sauf erreur ds les données
      if (da * ttodeath > 11) then
        sumhaz := sumhaz + (d + dn * lh.events[i + aux].tp) / da
      //le terme Exp est nul  l'integrale vaut d/da
      else if (da * ttodeath < 0.0001) then
...
begin
  ...
  begin
    ...
    else if t2 < tinf then if t1 < tc then
        censored_mort := 1 - survp(t2, su, sex) * Sda(t2, d, da, dn, hv, sex)
      else
        censored_mort := survp(t1, su, sex) * Sda(t1, d, da, dn, hv, sex) -
          survp(t2, su, sex) * Sda(t2, d, da, dn, hv, sex)
  ...
  begin
    if t2 >= tinf then  censored_mort := surv(t1, su, sex) * Sda(t1, d, da, dn, hv, sex)
      ...
      censored_mort := surv(t1, su, sex) * Sda(t1, d, da, dn, hv, sex) -
        surv(t2, su, sex) * Sda(t2, d, da, dn, hv, sex);
  end;
...
(****************************************************************)
function fx(var t, r: double; var legroupe: groupe_info): double;
  //ATTENTION SENESCENCE DE REPRO ratiotailleponte pas inclu ds le calcul
  //var matur,rate,survie : double;
begin
  //NB ma == matsanspon si matclutch = true et ma == mat sinon
  //matur:=1-surv(t,ma);
...
begin                                          { 0 ,  1 ,  ...,  nbevent-2, nbevent-1}
  ...
  begin
    ...
    if hv.events[I].Name = 'nop' then hv.events[I].debut := hv.events[I - 2].fin;
    { le derniere ponte est l'evenement I-2, au pire la maturite}
  end;
...
begin
  if matclutch = 1 then      //mode daphnie : la maturite est  une ponte
    if hv.events[1].tp = 0 then hv.nbponte := 0
    else
      hv.nbponte := 1 + hv.nb_event - 4;
  //attention modifié pour daphnie ou mat=1ere ponte  et compte ds nbponte
...
begin
  ...
  for i := 0 to nb_group - 1 do for j := 0 to group[i].nb_ind - 1 do
      if group[i].gi[j].lh[0].nbponte > 1 then
      begin
        nbind2pontes := nbind2pontes + 1;
        moy := moy + (group[i].gi[j].lh[0].events[2].fin -
          group[i].gi[j].lh[0].events[2].debut);
      end;
  if nbind2pontes > 0 then intervalp1p2 := moy / nbind2pontes
  else
    intervalp1p2 := 0;
...
begin
  ...
  begin
    ...
    // attention du fait du ratio cette esperance pourrait se retrouver <1 et ce n'est pas possible mathematiquement
    probevent :=
      (survtotpon(ev.t1 - ev.debut, ev.t1 - hv.events[1].fin, pon, hv, sex) -
      survtotpon(ev.t2 - ev.debut, ev.t1 - hv.events[1].fin, pon, hv, sex)) *
      ppoissontrunc(esptailleponte, ev.tp) + minus;
    //(Stot(t1) - Stot(t2))prob(taille ponte/integrale) attention pon.vp[2] n'est pas a proprement parler la moyenne de la loi de poisson tronquée qui vaut en fait vp3/(1-Exp(-vp3)): il faut reecrire la function poissontrunc pour que son parametre corresponde a son esperance directement
  end;
  //attention tfrommat calculé depuis ev.t1 car l'effet de snescence est pris en compte au debut de l'intervale
...
begin
  ...
  begin
    ...
    if Name = 'wei' then
    begin
      //NOTATIONS MATHEMATICA!!
      aux1 := Gamma(1 + 1 / vp[1]);
      aux2 := Gamma(1 + 2 / vp[1]);
      getvar := vp[0] * vp[0] * (aux2 - aux1 * aux1) / (aux1 * aux1);
      //Weibull[vp1,vp0/Gamma(1+1/vp1)]   ca donne une esperance de vp0 et la variance est compliquée mais donnée par getvar
    end;
    if Name = 'lgn' then getvar := vp[0] * vp[0] * (Expo(vp[1] * vp[1]) - 1);
    //LogNormale[0.5*(2Log[vp0]-vp1²) , vp1 ]  ca donne une esperane de vpO et la variance egale à   vp0²(Exp(vp1²)-1)
    if Name = 'gam' then getvar := vp[0] * vp[1];
    // Gamma[vp0/vp1,vp1]  ca donne une moyenne qui est vp0 et une variance vp0 vp1
  end;
end;
...
var
  varmat, varpon, varmatsanspon, diffmoy: double;
begin
  legroupe.matsanspon.Name := 'gam';
  diffmoy := legroupe.mat.vp[0] - legroupe.ponte.vp[0];
  legroupe.matsanspon.vp[0] := max(0.1, diffmoy);
  //difference des esperance  mat = matsanspon + pon
  legroupe.matsanspon.vp[2] := legroupe.mat.vp[2];

  varmat := getvar(legroupe.mat);
  varpon := getvar(legroupe.ponte);
  if varmat > varpon then varmatsanspon := varmat - varpon
  else
    varmatsanspon := 0.001;
...
begin
  ...
  begin { boucle sur les groupes }
    for j := 0 to nb_group - 1 do
      if group[j].use = 1 then
        // A FAIRE USE=1 si existe au moins 1 individu ds groupe
      begin
        //group[j].ll:=0;
        for k := 0 to 4 do
        begin
        ...
        for I := 0 to group[j].nb_ind - 1 do
          { boucle sur les individus ds groupe j }
        begin
          ans2 := 0;
          for m := 0 to group[j].gi[I].nb_hv - 1 do
            { boucle sur le nb histoire de vie individu i }
          begin
            ans3 := 1;
    ...
    begin { nst trials at a given temperature }
      ...
      begin { perturb each variable }
        ...
        begin
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      mars;
                      if (uni > 0) then acc := (ln(uni) < df)
                      else
                        acc := False; { this is the important line}
                    end;
                  end
                  else
                    acc := (df > 0);
          ...
          if acc { the change was accepted } then
          begin
...
begin
  ...
  until (Metrop_des.tempf <= 0) or (Metrop_des.climbrate <= 0) or
    (t = Metrop_des.maxrep);
  ...
  with Metrop_des do
    ...
    begin
      ...
      until (BestResult - LastResult < Metrop_des.PRECISION) or
        (rep > Metrop_des.maxrep);
...
begin
  ...
  xRatiomax := Ln(ratiomax - 1);
  //utile pour fonction CalculRatioEspPoissonTronque, pour ne pas avoir a le calculer tout le temps
...
begin
  ...
  begin
    ...
    // 'to(ps)int'            //tradeoff ponte -> surv intercept (evenement ponte augmente le hazard de survie de cette quantité)
    minBound := 0.00001;
    maxBound := 10;
    //   value := 0.5;
  end;
  ...
  begin
    Name := 'lin_decrease_hazard';
    // 'sen(pu)t'           //senescence de hazard ponte, effet lineaire du temps post-maturité
    minBound := -20;
    maxBound := 20;
    //   value := 0.5;
  end;

  with FD.paramdescript[15] do
    //senescence de hazard ponte, effet quadratique du temps post-maturité
  begin
  ...
  with FD.paramdescript[16] do
    //senescence de taille ponte, effet linéaire du temps post-maturité
  begin
  ...
  with FD.paramdescript[17] do
    //senescence de taille ponte, effet quadratique du temps post-maturité
  begin
  ...
  with FD.paramdescript[18] do
    //tradeoff intervale ponte i-1 qui change taille ponte i
  begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        LL_D.var_info[Count].Name :=
          'int_' + LL_D.paramdescript[i].Name;
      ...
      begin
        ...
        Setlength(ll_d.var_info, Count + 1);
        LL_D.var_info[Count].Name :=
          'slo_' + LL_D.paramdescript[i].Name + '_' + covar[modele[i].termcov0[j]].Name;
      ...
      begin
        modele[i].termcov0[j] := modele[i].term[j] mod 10;
        //ligne changée termcov0 initialement
        modele[i].termcov1[j] := modele[i].term[j] div 10;
        //ligne changée termcov1 initialement
        modele[i].firstvi[j] := Count + 1;
        ...
        if (covar[modele[i].termcov1[j]].typ = 1) and
          (covar[modele[i].termcov0[j]].typ = 0) then
          //facteur x continu
        begin
          Setlength(ll_d.var_info, Count +
        ...
        if (covar[modele[i].termcov1[j]].typ = 1) and
          (covar[modele[i].termcov0[j]].typ = 1) then
          //continu x continu
        begin
          Count := Count + 1;
          Setlength(ll_d.var_info, Count + 1);
...
begin
  for i := 0 to nb_group - 1 do with group[i] do
      for j := 0 to nbparposs - 1 do
        ...
        begin
          group[i].param[j].valpo[k] := 1;
          //valpo valent 1 si covar discrete, sinon, depend de valcont
          ...
          if (modele[j].term[k] > 0) and (modele[j].term[k] < 10) then
            ...
            begin
              group[i].param[j].po[k] := modele[j].firstvi[k];
              group[i].param[j].valpo[k] :=
                covar[modele[j].termcov0[k]].valcont[getcov(i, modele[j].termcov0[k])];
            end;
          ...
          begin
            ...
            //continu x facteur
            if (covar[modele[j].termcov0[k]].typ = 1) and
              (covar[modele[j].termcov1[k]].typ = 0) then
              if getcov(i, modele[j].termcov1[k]) = 0 then
                group[i].param[j].po[k] := -1
              else
              begin
                group[i].param[j].po[k] :=
                  modele[j].firstvi[k] + getcov(i, modele[j].termcov1[k] - 1);
                //le firstvi est le count+1, il faut donc firstvi+getcov-1   ; le getcov renvoie les valeurs 0 à level-1. Le cas zero est exclu d'entrée, les po commence a firstvi + getcov-1
                group[i].param[j].valpo[k] :=
                  covar[modele[j].termcov0[k]].valcont[getcov(i, modele[j].termcov0[k])];
              end;
            //facteur x continu
            if (covar[modele[j].termcov1[k]].typ = 1) and
              (covar[modele[j].termcov0[k]].typ = 0) then
              if getcov(i, modele[j].termcov0[k]) = 0 then
                group[i].param[j].po[k] := -1
              else
              begin
                group[i].param[j].po[k] :=
                ...
                //le firstvi est le count+1, il faut donc firstvi+getcov-1   ; le getcov renvoie les valeurs 0 à level-1. Le cas zero est exclu d'entrée, les po commence a firstvi + getcov-1
                group[i].param[j].valpo[k] :=
                  covar[modele[j].termcov1[k]].valcont[getcov(i, modele[j].termcov1[k])];
              end;
            //continu x continu
            if (covar[modele[j].termcov1[k]].typ = 1) and
              (covar[modele[j].termcov0[k]].typ = 1) then
            begin
              group[i].param[j].po[k] := modele[j].firstvi[k];
              group[i].param[j].valpo[k] :=
                covar[modele[j].termcov0[k]].valcont[getcov(i, modele[j].termcov0[k])] *
...
begin
  ...
  with FD do { Write out the results }
    for i := 1 to number_of_variables do
    begin
      Write(outfile, var_info[i].Name, ' ', var_info[i].Value: 10: 8,
        ' ', var_info[i].se: 10: 8);
      writeln(outfile);
    end;
...
begin
  ...
  repeat { a cooling process over ntr loops }
        ...
        begin
          ...
          begin
            ...
            begin
              Value := Value + step * (uni - 0.5);
              { should we ACCept this ATTempted change ? }
              if (Value < minBound) or (Value > maxBound) then ACC := False
              else
              begin
                newFresult := f(fun_des);
                df := (newFresult - CurrResult);
    ...
    for j := 1 to fun_des.number_of_variables do
      with fun_des.var_info[j] do sample[t - 1] := Value;
    //sampling at each of the ntr loops
