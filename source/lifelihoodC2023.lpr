...
begin
  ...
  try
    ...
    //randon generator
    seed1 := StrToInt(list[9]);
    savedseed[1] := seed1;
    seed2 := StrToInt(list[10]);
    savedseed[2] := seed2;
    seed3 := StrToInt(list[11]);
    savedseed[3] := seed3;
    seed4 := StrToInt(list[12]);
    savedseed[4] := seed4;
    ...
    sub_interval := StrToFloat(list[16]);
    ...
    //MCMC
    if StrToInt(list[3]) > 0 then
    begin
      nbsample := StrToInt(list[3]);
      intervalsamples := StrToInt(list[4]);
    ...
    //ecriture outfile
    printout_FD(LL_D, nomf1, StrToInt(list[3]), list[5]);
