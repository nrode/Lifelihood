...
_Note: this vignette assumes that you already have your data in the right format. If not, check the [vignette about the required data format](1-required-data-format.html)_
...
There are 2 main sections in the required `YAML` configuration file:

- constants: some fixed parameters, set by design
- estimations:
...
- `right_censoring_date` (integer): time point at which a subject's data is censored. This means that for subjects who do not experience the event of interest (e.g., death, failure) by this date, their data is considered censored. In practice, choose a value much larger than the maximum longevity seen in the data.

- `critical_age` (integer): critical age below which life histories are not followed individually.

- `ratiomax` (integer): maximum ratio between number of offspring of last and first reproduction events. Cannot be greater than ratiomax.
...
- `1 + covar1`: fit the intercept and the parameter of `covar1`.
- `1 + covar1 + covar2`: fit the intercept, the parameter of `covar1` and the parameter of `covar1`.
- `1 + covar1 + covar2 + covar1*covar2`: fit the intercept, the parameter of `covar1`, the parameter of `covar1` and the interaction between `covar1` and `covar2`.
...
- `expt_reproduction`: Expected time before next reproduction
- `reproduction_shape`: Reproduction shape parameter
- `n_offspring`: Number of offsprings per reproduction event or fitness
- `increase_death_hazard`: Increase in death hazard following a reproduction event
- `tof_reduction_date`: Trade-off of reduction date
...
- `quad_senescence`: Quadratic senescence of interval between reproduction events
