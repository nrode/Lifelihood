...
find_event_type <- function(metric_name) {
  ...
  } else if (metric_name %in% c("expt_reproduction", "reproduction_shape", "n_offspring", "increase_death_hazard", "tof_reduction_date", "increase_tof_n_offspring", "lin_decrease_hazard", "quad_decrease_hazard", "lin_change_n_offspring", "quad_change_n_offspring", "tof_n_offspring")) {
    event_type <- "reproduction"
  } else if (metric_name %in% c("expt_maturity", "maturity_shape", "ratio_expt_maturity")) {
    event_type <- "maturity"
  } else {
    stop("Invalid metric_name value.")
  }
  return(event_type)
}
