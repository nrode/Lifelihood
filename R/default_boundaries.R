...
default_bounds_df <- function(lifelihoodData) {
  ...
  reproduction_model <- dist[["reproduction"]]
  ...
  boundaries <- list(
    ...
    maturity_param2 = c(
      name = "maturity_param2",
      min = maturity_param2_min,
      max = maturity_param2_max
    ),
    ...
    tof_decay = c(name = "tof_decay", min = 1e-07, max = 10),
    increase_death_hazard_n_offspring = c(
      name = "increase_death_hazard_n_offspring",
      min = 1e-07,
      max = 10
    ),
