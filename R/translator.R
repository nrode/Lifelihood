...
decode_file_with_translator <- function(output_path, translator) {
    ...
    function(line) {
      for (colname in names(translator)) {
        ...
        line <- gsub(
          pattern,
          function(m) {
            num <- as.integer(sub(paste0(colname), "", m))
            levels <- translator[[colname]]

            if (num + 1 > length(levels) || num < 0) {
              warning(paste("Invalid index for", colname, ":", num))
              return(m) # leave unchanged
            }

            paste0(colname, levels[num + 1])
          },
          line,
          perl = TRUE
        )
  ...
  # Overwrite or return
  writeLines(decoded_lines, output_path)
  invisible(decoded_lines)
}
